...
export default function HomePage() {
  ...
  const degreesCloseTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    if (!isDegreesOpen && !isQualOpen) return;
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <div
              ...
              onMouseEnter={() => {
                if (degreesCloseTimer.current) clearTimeout(degreesCloseTimer.current);
                setIsDegreesOpen(true);
              }}
              onMouseLeave={() => {
                degreesCloseTimer.current = setTimeout(() => setIsDegreesOpen(false), 150);
              }}
            >
              <button
            ...
            <div
              ...
              className="relative hidden sm:block"
              onMouseEnter={() => {
                if (qualCloseTimer.current) clearTimeout(qualCloseTimer.current);
                setIsQualOpen(true);
              }}
              onMouseLeave={() => {
                qualCloseTimer.current = setTimeout(() => setIsQualOpen(false), 150);
              }}
            >
              <button
                className="flex items-center gap-1 text-sm font-medium hover:opacity-70"
                style={{ color: isScrolled ? "#000000" : "#FFFFFF", transition: "color 0.3s", background: "none", border: "none", cursor: "pointer", padding: 0 }}
                onClick={() => setIsQualOpen(v => !v)}
              >
                국가자격증
                <ChevronDown
                  size={14}
